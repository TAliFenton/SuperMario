...
#include"PlayerDarioPart.h"
#include"CoinDarioPart.h"
...
int main()
{
    ...
    if (!music.openFromFile(resourcePath()+"images/marioSong.ogg"))// import mario theme song
        cout << "Could not load file\n";
    
    if (!texture.loadFromFile(resourcePath()+"images/NES2.png")|| !title.loadFromFile(resourcePath()+"images/marioTitle6.png"))// import our background and our main mario page
        cout << "Could not load file\n";
    /*
    ...
    while (window.isOpen())// game loop
    {
        ...
        cout << "Mario's Position: " << Mario.getPositionY() << endl;
        while (window.pollEvent(event))// check for events
        {
            if (event.type == sf::Event::Closed)// if you would like to close the window
                window.close();
